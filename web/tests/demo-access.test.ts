...
describe("private demo access requests", () => {
  ...
  it("approves once, queues without delivery, and revokes an unclaimed invite", async () => {
    ...
    const approved = await D.approveDemoAccessRequest({ requestId: request.id, actor: { id: "access-template-owner", email: "OWNER@EXAMPLE.COM" } });
    assert.equal(approved.ok, true);
    if (!approved.ok) return;
    assert.match(approved.token, /^[A-Za-z0-9_-]{43}$/);
    const invite = await C.queryOne<any>("SELECT token_hash,request_id FROM beta_access_invites WHERE request_id=$1", [request.id]);
    assert.equal(invite.request_id, request.id);
    ...
    assert.equal(await D.revokeDemoAccessRequest({ requestId: request.id, actor: { id: "access-template-owner", email: "owner@example.com" } }), true);
    assert.deepEqual(await D.approveDemoAccessRequest({ requestId: request.id, actor: { id: "access-template-owner", email: "owner@example.com" } }), { ok: false, reason: "not_pending" });
