...
export async function ingestResearch(input: {unitid:string;term:string;attemptId:string;candidates:Candidate[]}): Promise<{changed:boolean; states:Resolution[]}> {
  ...
  return withTransaction(async () => {
    ...
    await exec("UPDATE school_research_jobs SET last_checked_at=$1,next_check_at=$2,material_fingerprint=$3,publication_revision=publication_revision+$4,updated_at=$5,first_evidence_committed_at=COALESCE(first_evidence_committed_at,$6) WHERE unitid=$7 AND term=$8 AND attempt_id=$9",[now,next,fingerprint,changed?1:0,now,now,input.unitid,input.term,input.attemptId]);
