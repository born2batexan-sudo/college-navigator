...
export function sealTokens(tokens: {accessToken:string;refreshToken:string}): string {
...
function openTokens(sealed: string): {accessToken:string;refreshToken:string} {
...
export async function storeConnectedMail(input: {householdId:string;actorId:string;provider:MailProvider;accountId:string;tokens:{accessToken:string;refreshToken:string};consentVersion:string}) {
...
export async function reconsentConnectedMail(input:{householdId:string;actorId:string;connectionId:string;accountId:string;tokens:{accessToken:string;refreshToken:string};consentVersion:string}) {
 await assertOwner(input.householdId,input.actorId);
 ...
 await withTransaction(async()=>{
  ...
  await exec("UPDATE mail_connections SET encrypted_tokens=$1,key_version='v1',consent_version=$2,consented_by=$3,consented_at=$4,status='active',checkpoint=NULL,last_sync_at=NULL,revoked_at=NULL,delete_after=NULL WHERE id=$5",[sealed,input.consentVersion,input.actorId,nowIso(),row.id]);
...
export async function syncConnectedMail(input: {householdId:string;actorId:string;connectionId:string;adapter:ConnectedMailAdapter}) {
 ...
 try { batch=await input.adapter.listMinimalMetadata(scope,openTokens(connection.encrypted_tokens)); }
  ...
  const state=/consent|invalid.grant|unauthor/i.test(String(e))?'reconsent':'failed';
 ...
 if (!Array.isArray(batch.envelopes) || batch.envelopes.length>50 || !batch.nextCheckpoint || batch.nextCheckpoint.length>500) throw new Error('Invalid adapter batch');
 return withTransaction(async()=>{
  ...
  for (const envelope of batch.envelopes) {
   ...
   const matched=senders.filter(s=>s.domain===sender);
...
export async function revokeConnectedMail(input:{householdId:string;actorId:string;connectionId:string}) {
 ...
 await withTransaction(async()=>{
  ...
  await control(input.householdId,'locally_revoked',input.actorId,'remote_revocation_pending');
 });
}
