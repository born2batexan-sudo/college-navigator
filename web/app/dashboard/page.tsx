...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="flex flex-col gap-8">
    ...
    {!selectedPlan ? <>
      ...
      <section><h2 className="mb-4 font-display text-2xl font-semibold text-ink">Student plans</h2><div className="grid gap-4 md:grid-cols-2">{plans.map((plan) => { const counts = actionCounts(plan.actions); return <article key={plan.id} className="rounded-2xl border border-line bg-white/80 p-5 shadow-card"><div className="flex items-start justify-between gap-3"><div><h3 className="font-display text-xl font-semibold text-ink">{plan.name}</h3><p className="mt-1 text-sm text-ink/55">{plan.relationships} school{plan.relationships === 1 ? "" : "s"} tracked · {plan.term ?? "start term not set"}</p></div><Link href={studentUrl(plan.id)} className="text-sm font-semibold text-accent underline">Open plan</Link></div><dl className="mt-4 grid grid-cols-3 gap-2 text-center text-sm"><div><dt className="text-ink/45">This week</dt><dd className="mt-1 font-semibold text-urgent">{counts.due.length}</dd></div><div><dt className="text-ink/45">Waiting</dt><dd className="mt-1 font-semibold text-warn">{counts.waiting.length}</dd></div><div><dt className="text-ink/45">On track</dt><dd className="mt-1 font-semibold text-ok">{counts.onTrack.length}</dd></div></dl></article>; })}</div></section>
      <section><h2 className="mb-3 font-display text-2xl font-semibold text-ink">Household action queue</h2>{allActions.length === 0 ? <p className="rounded-2xl border border-dashed border-line bg-white/40 p-7 text-center text-sm text-ink/50">Nothing open right now.</p> : <div className="flex flex-col gap-2">{allActions.slice(0, 25).map((action) => <ActionListItem key={action.id} action={action} schoolName={`${plans.find((plan) => plan.actions.includes(action))?.name ?? "Student"} · ${action.schoolName}`} />)}</div>}</section>
