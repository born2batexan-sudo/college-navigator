...
export default async function RequestSchoolPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  // Server-side authenticated render handoff, not proof of browser receipt.
  ...
  return (
    <main className="flex max-w-3xl flex-col gap-7">
      ...
      {query.submitted && <p role="status" className="rounded-md border border-ok/30 bg-ok/10 p-3 text-sm text-ok">Your request is saved. If research is enabled, check this page for a partial first evidence view; unresolved subjects will not become a verified plan automatically.</p>}
