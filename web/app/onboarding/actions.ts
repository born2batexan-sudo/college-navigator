...
export async function saveOnboarding(formData: FormData): Promise<void> {
  ...
  if (names.length !== parsed.data.studentCount || terms.length !== parsed.data.studentCount) redirect("/onboarding?error=Add+a+profile+for+every+student+you+selected.");

  const students = names.map((name, index) => studentSchema.safeParse({ name, enteringTerm: terms[index] }));
